...
def parse_extensions(data):
    ...
    try:
        ...
        print(f"Error occurred while parsing extensions: {e}")
    return extensions
...
def detect_tls_version_by_keyshare(server_hello):
    ...
    if len(server_hello) < 5:
        raise ValueError("Server hello data is too short to parse TLS record layer.")
    ...
    if len(handshake_data) < 4:
        raise ValueError("Handshake data is too short to parse handshake header.")
    ...
    if handshake_type != 0x02:  # 0x02表示Server Hello
        raise ValueError("Not a Server Hello message")
